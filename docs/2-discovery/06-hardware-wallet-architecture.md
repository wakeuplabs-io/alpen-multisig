...
> **Resolved (2026-04-17).** All four questions below were answered during POC-5 and the subsequent library analysis. See [`07-hardware-wallet-library-analysis.md`](./07-hardware-wallet-library-analysis.md) and [`15-poc5-trezor-findings.md`](./15-poc5-trezor-findings.md).
...
| 2 | Which Rust crate handles Trezor protobuf? (`trezor-client` or custom) | **Resolved** — `trezor-client` (upstream Trezor Rust crate, HID transport) is used by the POC-5 driver. See [`15-poc5-trezor-findings.md`](./15-poc5-trezor-findings.md) and `desktop-app/src-tauri/src/infrastructure/hw_wallet/trezor.rs`. |
| 3 | Does the device need to show a human-readable summary on screen, or just a hex sighash? Determines whether to use `signMessage` vs `signPsbt`. | **Resolved** — strict SPS-65 requires bare ECDSA over the 32-byte tagged sighash, which no consumer-Bitcoin firmware exposes directly. POC-5 documents the synthetic-PSBT workaround. On-device display remains a UX constraint, not a signing-API one. See [`07-hardware-wallet-library-analysis.md`](./07-hardware-wallet-library-analysis.md) §4 and [`15-poc5-trezor-findings.md`](./15-poc5-trezor-findings.md) §1–§3. |
| 4 | For the address display flow (`connect()`) — do JS SDKs stay for this step only, or does device discovery also move to Rust? | **Resolved** — all device I/O (discovery, address, signing) moved to Rust inside Tauri. JS SDKs were dropped; see ADR-005 and [`15-poc5-trezor-findings.md`](./15-poc5-trezor-findings.md). |

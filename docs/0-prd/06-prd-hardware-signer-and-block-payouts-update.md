...
3.  1. The user MUST be able to select the multisig they want to interact with. The supported multisigs MUST be:
    ...
    2. The user MUST be able to connect a supported hardware signer to sign messages and transactions with.
       1. Supported hardware signers MUST include all hardware signers currently supported by [HWI](https://github.com/bitcoin-core/HWI/blob/master/docs/devices/index.rst) using the following features:
          - Taproot inputs,
          - BIP-84 message signing,
          - Display on device screen, and
          - Are otherwise compatible with SPS-65 updates.
       1. Admin ID: 
          1. If the user selected the Payout Administrator in the previous step, then the application MUST use the user’s connected hardware signer to generate an “Admin ID” represented by a P2TR bitcoin address generated using the [BIP-86-compliant](https://bips.dev/86/) `m/86'/0'/73'/0/0` derivation path, where `73'` is the hardened account used to generate the Admin ID, and `/0/0` are the change and address indexes, respectively. The Admin ID MUST be used for authenticating with the multisig app backend and signing all Payout Administrator transactions, as described in Requirement 6.
          1. If the user selected any multisig other than the Payout Administrator in the previous step, then the application MUST use the user’s connected hardware signer to generate an “Admin ID” represented by a P2WPKH bitcoin address generated using the [BIP-84-compliant](https://bips.dev/84/) `m/84'/0'/73'/0/0` derivation path of the connected hardware signer, where `73'` is the hardened account used to generate the Admin ID, and `0` is the address index. The Admin ID MUST be used for authenticating with the multisig app backend and signing all admin subprotocol update-related messages. For the avoidance of doubt, the Admin ID MUST NOT be used to sign any bitcoin transactions.
       1. Admin Wallet: The application MUST use the user’s connected hardware signer to generate an “Admin Wallet” using the [BIP-86-compliant](https://bips.dev/86/) `m/86'/0'/73'/n/n` derivation path, where `73'` is the hardened account used to generate the Admin Wallet, and `/n/n` are the change and address indexes, respectively.
       1. The user MUST be able to clearly read and understand each message they are signing on their hardware signer screen, to be able to visually verify that the message they are signing matches what they are expecting based on what they are seeing in the application UI.
    3. After connecting their Admin ID and Admin Wallet, the user MUST sign a nonce with the private key of their Admin ID to gain access to the UI for the selected multisig.
       1. After connecting their hardware signer, but before signing in, the user MUST be able to click a “Verify” button next to the Admin ID that enables them to: 
          - View their Admin ID
          - Generate an “Admin ID Verification Certificate” by signing their Admin ID with the Admin ID’s private key
          - Copy the generated Admin ID Verification Certificate to their clipboard, 
          - And click a button to view / verify the Admin ID on their hardware signer screen

so that they can propose to have the Admin ID added to the canonical list of multisig signers.

See wireframes for this feature [here](https://drive.google.com/drive/folders/1O9wgZ-UwvJ03VRuaaDadKnSpcW0ySnr1?usp=drive_link).
          1. Anyone with the Admin ID Verification Certificate MUST be able to independently use the Admin ID Verification Certificate to derive the Admin ID’s compressed public key.
...
4.  1. After logging in to the application, the user MUST be able to view their Admin ID, generate an Admin ID Verification Certificate, and view their Admin ID on their hardware signer screen, as described in Requirement (3.c.i) above and shown in wireframes [here](https://drive.google.com/drive/folders/1O9wgZ-UwvJ03VRuaaDadKnSpcW0ySnr1?usp=drive_link).
    2. The user MUST be able to manage their Admin Wallet using the application, including:
       ...
       1. Transactions: The user MUST be able to see each unconfirmed transaction sent from the Admin Wallet and have the ability to bump the fee rate. 
       1. Receive: see the first unused address in the wallet’s `address` index, in both text and QR code formats.
          1. Clicking on the address text or QR code MUST copy the address to the user’s clipboard.
          1. The user MUST be able to click a button to view their receive address on their hardware signer screen to verify that the address they see in the UI was actually derived from the seed phrase loaded in their connected hardware signer.
       ...
       1. Send BTC:
          ...
          1. There MUST be a "Confirm" button that is disabled by default, and is only enabled when all send form text entry fields are correctly filled.
             1. Clicking the "Confirm" button when it is enabled MUST send a bitcoin transaction to the user’s connected hardware signer; if the user confirms the transaction on their hardware signer, then the transaction MUST be sent to the bitcoin network for confirmation and the user MUST be shown the transaction ID; if the user rejects the transaction in their hardware signer then nothing should happen in the UI and the user can either click “Confirm” to try again or else back out of the “send” screen.
5.  1. The requirements in this section MUST only apply to the following multisigs, unless explicitly stated otherwise:
    ...
    2. The user MUST be able to see all "Approved" updates and how many cancellation signatures each "Approved" update has received (if any). An "Approved" update is an update that has reached the required quorum of approval signatures and has been confirmed onchain, but has not yet been enacted.
       ...
       1. For the avoidance of doubt, this subsection (b) does not apply to the following multisigs / transaction type, because they do not produce proposals that have an "Approved" or "Canceled" state:
          - Strata Sequencer Manager multisig
...
6.  1. The requirements in this section MUST only apply to the Payout Administrator multisig, unless explicitly stated otherwise.
    2. The user MUST be able to see all "Pending" `block_payouts` transactions, along with the following metadata about the transactions: 
       ...
       1. The user MUST see an informational message on the relevant transactions if two or more “Pending” `block_payouts` transactions contain one or more of the same inputs.
          1. The specific inputs that are the same should have an informational icon next to them, with a tooltip that says: “This input is included in multiple Pending transactions. To prevent double-spending, only one of these transactions can be confirmed.”
...
<list of invalid signatures>”
    ...
    4. The user MUST be able to create a new `block_payouts` transaction by clicking a "Block payouts" button.
       1. The user MUST be able to provide one or more “false claim reports”, from which the application can derive the `block_payouts` outpoints to be added as inputs to the `block_payouts` transaction.
          ...
          1. The application MUST ignore any `block_payouts` outpoints that have already been spent in a confirmed transaction.
       1. The application MUST accept unspent `block_payouts` outpoints as inputs until the transaction has reached the standard transaction size limit according to the latest release of Bitcoin Core, accounting as well for the fee input(s) and one change output to a change address in the user’s Admin Wallet.
          1. If any outpoints are excluded due to the transaction reaching the standard transaction size limit, then the application should create as many transactions as needed to spend all outpoints with valid corresponding false claim proofs, applying the fee rate specified by the user as described below to each transaction.
       1. The user MUST be able to specify a fee rate in s/VB and in increments of 0.1 s/VB up to 10,000 s/vB.
          ...
          1. The user MUST see a preview of the transaction fee given the specified fee rate.
          1. The user MUST receive a critical error message if their Admin Wallet does not have sufficient funds to pay the given transaction fee.
             1. “Admin Wallet has insufficient funds to pay this transaction fee.”
       1. For each transaction being created, the user MUST see each of the `block_payouts` inputs that are included in the transaction, and a numeric total for the number of `block_payouts` inputs.
          1. The user MUST be able to remove individual `block_payouts` inputs from the transaction.
       1. The user MUST receive a critical error message if the size of the transaction exceeds standardness limits in the most recent release of Bitcoin Core.

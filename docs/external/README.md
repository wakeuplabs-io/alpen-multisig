...
4. **Current Scope** — Documentation reflects the current implementation status. Some features (e.g., Alpen Administrator VK update enactment detection) are planned for later phases — see [Architecture Overview](./architecture-overview.md) limitations and open items described there.

...
**Last updated:** 2026-09-22 (Stage 6 audit against `develop` after #570–#572; rows 14 and 16 updated when the cancel gate and the expiry test landed)
...
| 14 | §5.3.3 | A pending update expires after 7 days | **PASS** *(general)* | `expire_if_overdue` `orchestrator-be/src/application/proposals.rs:291`, on read from `handlers/proposals.rs:162`/`:211`; default 7 days `config.rs:76` · `expire_if_overdue_expires_only_a_pending_proposal_past_its_window` | The window is `PROPOSAL_EXPIRY_DAYS`, 7 unless a deployment overrides it; a deployment that sets it lower does not meet §5.3.3. |
...
| 16 | §5.2 | "Approved" means quorum **and** confirmed on chain | **PARTIAL** *(general, documented deviation)* | `proposal-status.ts:75` shows *Approved* once quorum is reached, before the broadcast; after the reveal confirms it shows *Awaiting enactment* | The application's *Approved* is the backend status (quorum), one step earlier than the PRD's. It is a label: the cancel affordance itself waits for the update to be queued (`canCancelProposal`, `desktop-app/src/domain/proposal-detail/model/derive-proposal-actions.ts:38`), on every surface. |
...
| 16 | *Approved* shown at quorum, before on-chain confirmation | — |

...
| **D4** | Default fee = **Fast** preset (`target_blocks == 1`) | PRD §4.3.5.3: default MUST be the node's "next block" rate. Phase 4's `useFeePresets` defaults to `medium` for governance ([`governance-broadcast-fee-selection.md`](./governance-broadcast-fee-selection.md)) — Send overrides the initial selection to `fast` |

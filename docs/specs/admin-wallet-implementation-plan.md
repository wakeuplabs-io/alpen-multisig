...
| R1.3 ✅ | Receive rotation | PRD §4.3.4 **rotation only** (PARTIAL in compliance matrix); QR/HW verify → Phase 6; [`admin-wallet-receive-rotation.md`](./admin-wallet-receive-rotation.md) |
...
The plan has three parts: the completed **Foundation** (Phases 1–3.8), **Release 1** (R1.0–R1.6 engineering complete — PR [#211](https://github.com/wakeuplabs-io/alpen-multisig/pull/211) R1.5, PR [#212](https://github.com/wakeuplabs-io/alpen-multisig/pull/212) R1.6; PRD §4.3.1–§4.3.2 **PASS** per [`admin-wallet-prd-compliance.md`](./admin-wallet-prd-compliance.md)), and the **Remaining phases (4–9)**.
...
**Out of scope:** Production Send, tx list, QR, HW verify (Phases 4–7); PRD §4.1–4.2 Admin ID UI (Phase 6).
...
**Goal:** Issue a fresh receive address and rotate after credit (PRD §4.3.4.3). QR and verify-on-device remain Phase 6 — see compliance matrix **PARTIAL** for §4.3.4 overall.
...
Phases 4–9 continue after Release 1 (including R1.5–R1.6). **Phase 6 (receive QR) and Phase 7 (HW Send) overlap with Release 1** — their entries below list only what remains after Release 1 ships.
...
| Chain access | `HttpBitcoinRpcClient` in `infrastructure/bitcoin_rpc.rs` | Unchanged; wallet sync uses mempool emitter (R1.5) |
...
| Admin ID HW | BIP-84 paths in adapters | Canonical connect (R1.4); **PRD §4.1–4.2 UI still FAIL** (Phase 6) |
...
| BDK | Not in workspace | `bdk_wallet`, `bdk_bitcoind_rpc` |
| Product RPC assumption | Local regtest `bitcoind`; `BITCOIN_WALLET_NAME` for legacy commit | Local `bitcoind` for dev/CI; `BITCOIN_WALLET_NAME` / `COMMIT_FUNDING` removed (3.6); Node Config default local (PRD §2.2) until Phase 9 |

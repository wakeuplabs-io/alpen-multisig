...
| 4.2 | View Admin ID on HW to verify | **PARTIAL** | `VerifyOnDeviceButton` (`AdminIdRow`) | The device confirms the key **indirectly**: Trezor and Ledger cannot render a raw compressed public key — they display addresses (or an xpub). The button shows the P2WPKH address derived from the same key and path, with copy stating exactly that. See [`admin-wallet-admin-id-and-receive-qr.md`](./admin-wallet-admin-id-and-receive-qr.md) §Device capability (#409) |

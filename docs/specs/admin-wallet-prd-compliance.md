...
**Last updated:** 2026-08-19 (G10 — the login challenge is readable on **both** signers, Trezor measured by hand (`issues/evidence/G10-B2-TREZOR-LOGIN.md`); §3.2.4 notes updated, evidence `issues/evidence/G10-B0-CHALLENGE-MEASUREMENT.md` and `G10-B2-DEVICE-QA.md`. G9 closed §3.c.i and §4.2; playbook [`admin-id-verification-certificate-manual-playbook.md`](./admin-id-verification-certificate-manual-playbook.md), evidence `G9-B0-LEDGER-MEASUREMENT.md` and `G9-B1-EVIDENCE.md`)
...
| 3.2.4 | Readable messages on HW screen | **PARTIAL** | Message signing on connect; G9 B0 and G10 B0/B2 measurements — `issues/evidence/G9-B0-LEDGER-MEASUREMENT.md`, `G10-B0-CHALLENGE-MEASUREMENT.md`, `G10-B2-DEVICE-QA.md` | **Resolved in G10, now measured on both signers:** the session-authentication message now uses ` | ` separators and a Ledger renders it in full — the newlines were the sole trigger for the `Message hash` screen, and the signature over the displayed text verifies with the shipped verifier. Certificate signing was already text (G9). **Still PARTIAL:** the governance PSBT preview has never had a §3.2.4 audit of its own, and messages over 640 bytes (roughly six signer-set changes) still fall back to the hash by design of the Bitcoin app. Trezor measured 2026-08-19 (`issues/evidence/G10-B2-TREZOR-LOGIN.md`): the challenge renders as text across two pages and reassembles character for character — it paginates without `…` markers, so a truncation would have been silent — and the session authenticated |
...
| 4.2 | View Admin ID on HW to verify | **PASS** | G8 — `VerifyOnDeviceButton` as Step 2 of `AdminIdCertificateModal`; [`admin-id-verification-certificate.md`](./admin-id-verification-certificate.md); G9 device QA — `g9-certificate-{ledger,trezor}.qa.js`, `issues/evidence/G9-B1-EVIDENCE.md` | The indirection is gone: with the Admin ID being the address, the device renders the Admin ID **itself**, and the app checks the returned string matches (mismatch = security alarm). G8 moved this from the Admin ID card into the certificate modal, where PRD 06 §3.c.i puts it, and made it reachable **before** sign-in as well. A mnemonic session gets the step disabled with the reason — there is no device screen to compare against. **Post-login surface confirmed 2026-08-19** (`issues/evidence/g9-audit-f3-*.png`): the same Admin ID, the same device confirmation, and a certificate byte-identical to the pre-login one — Trezor signs deterministically, so that proves both surfaces derive through the same path (G9 audit F-3). **Flipped to PASS by G9**, which drove the shipped modal against both signers and asserted on what the device painted: Ledger (Speculos, Bitcoin app 2.4.2) shows `Address` + the Admin ID; Trezor (T2B1, fw 2.8.7) shows it under its generic `Receive address` heading — a firmware label, called out in the playbook because the Admin ID must never receive funds |
| 3.c.i | Admin ID Verification Certificate | **PASS** | G8 — `admin_id_certificate.rs`, `AdminIdCertificateModal`; unit + e2e specs; G9 device QA — `issues/evidence/G9-B1-EVIDENCE.md` | The certificate is the signature over `Admin ID: <address>` in Bitcoin Core's `signmessage` encoding (header `31 + recid`, the byte the normative wireframe pins), it is **self-verified in Rust before it is ever shown** — the recovered key must re-derive the Admin ID — and the copy button puts message and signature on the clipboard as two lines a reader can paste straight into a verifier. Independently checked against Bitcoin Core v28.1: `verifymessage <P2PKH from the same key> <certificate> "Admin ID: …"` → `true`, `false` on a tampered message. **Note:** Core's `verifymessage` rejects bech32 addresses outright (`Address does not refer to key`), so a certificate is verified against the P2PKH form of the same key, or by any tool that verifies through key recovery. **Flipped to PASS by G9:** the requirement's third bullet — reading the Admin ID off the signer's screen — was measured on both devices, and the Ledger renders the message as **text**, not a hash (`G9-B0-LEDGER-MEASUREMENT.md`) |
...
| 4.3.4.2 | Verify receive address on HW | **PASS** | `receive-address-row.tsx` — `VerifyOnDeviceButton` on the receive address (P2TR, receive path); manual Trezor run 2026-08-19, `issues/evidence/g9-audit-f4-*.png` | The device renders the receive address itself and the app compares the returned string. Trezor showed `bc1py0kl…` for the panel's `bcrt1py0kl…`: **only the HRP differs**, because the device firmware carries no regtest coin — the panel says so inline. Was marked FAIL / "Phase 8" long after the affordance shipped; caught by the G9 audit pass (F-4) |
...
| §3.c.i, §4.1–4.2 | G7 (address) + G8 (certificate) + G9 (device QA) | **PASS** ✅ |
...
**Release 1:** R1.0–R1.7 done. **Release 2:** R2.1–R2.3 done ✅. **Phase 4:** done ✅. **Phase 5:** done ✅. **Phase 6:** done ✅ (Send — regtest/dev mnemonic). **Phase 7:** done ✅ (Admin ID display/copy + receive QR & click-to-copy). **Admin ID program (PRD 06):** G7, G8 and G9 done ✅ — §4.1, §4.2 and §3.c.i all PASS, with device evidence. **Next:** Phase 8 (HW direct adapters — Send-on-HW; §4.2 was closed by G9 and §4.3.4.2 by the G9 audit pass, both with device evidence).

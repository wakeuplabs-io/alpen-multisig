...
**Last updated:** 2026-08-18 (G9 — device QA on Ledger and Trezor closed §3.c.i and §4.2; playbook [`admin-id-verification-certificate-manual-playbook.md`](./admin-id-verification-certificate-manual-playbook.md), evidence `issues/evidence/G9-B0-LEDGER-MEASUREMENT.md` and `G9-B1-EVIDENCE.md`)
...
| 3.2.4 | Readable messages on HW screen | **PARTIAL** | Message signing on connect; G9 B0 measurement — `issues/evidence/G9-B0-LEDGER-MEASUREMENT.md` | Governance PSBT preview not a full §3.2.4 audit. **Measured in G9:** a Ledger renders the message **text** for the certificate, and falls back to the SHA-256 `Message hash` only for non-printable or unpaginatable messages — not, as G3 concluded, because the build lacks a text branch. The **session-authentication** message still hashes, and the trigger is its embedded newlines (`auth_crypto.rs:10-12`): the identical 135-char string with spaces renders as text. Making that message readable is a change to a pinned signing contract → **G10** |

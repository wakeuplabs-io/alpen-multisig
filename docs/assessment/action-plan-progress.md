# Wave 1 action plan — progress tracker

Branch: `fix/action-plan-wave1-2026-05-14`  
Source: [action-plan-2026-05-14.md](action-plan-2026-05-14.md)

Manual E2E before each issue commit: `/e2e-proposal-flow` (operator confirms pass/fail).

| P-ID | Status | Commit | E2E | Notes |
|------|--------|--------|-----|-------|
| P-061 | done | 3981e0c | pending | Route broadcast via orchestrator HTTP |
| P-062 | done | fc3fbb9 | pending | Re-fetch proposal after broadcast |
| P-002 | done | 8178b15 | pending | Authority filter on list/get/broadcast |
| P-001 | done | 31b9f5a | pending | Reject missing/test operator secret key |
| P-015 | done | ceeeb7a | pending | Remove VITE operator key path |
| P-063 | done | 53bcd98 | pending | Pubkey case normalization |
| P-013 | done | 766d685 | pending | Explicit bitcoin network |
| P-014 | done | e51daa7 | pending | HTTPS on orchestrator URL |
| P-035 | done | becf9fe | pending | Threshold drift at broadcast |
| P-020 | done | 1261980 | pending | Broadcast in-flight guard |
| P-004 | done | 809789a | pending | Strict CSP |
| P-008 | done | 50d3d51 | pending | Zod IPC; nullish Option fields (fix atop 16df77b) |
| P-009 | done | 0556b5e | pending | Session bound to authority |
| P-010 | done | 5945d3a | pending | Sign deep-link authority guard |
| P-007 | done | 0825a1a | pending | Freeze sighash at preview |
| P-064 | done | 83399c6 | pending | Full Authority enum parity |
| label-bug | done | 2b6c1d4 | pending | Strata Administrator label |
| P-016 | done | 805c3e1 | pending | DATABASE_URL required in prod |
| P-029 | done | 0e2a34f | pending | Tracing skeleton + /ready |
| P-054 | done | 5bdf8bf | pending | Consolidate Cursor/Claude rules |

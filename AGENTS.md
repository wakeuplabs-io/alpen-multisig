...
See [`docs/README.md`](docs/README.md) for conflict resolution and topic index.
...
- **`orchestrator-be`** — Offchain coordination backend (Axum HTTP). Domain-driven layout: `domain/`, `application/`, `infrastructure/`, `handlers/`, `state.rs`, `config.rs`, `error.rs`. Entry: `src/main.rs`. Postgres when `DATABASE_URL` is set; in-memory repo for local dev without it.

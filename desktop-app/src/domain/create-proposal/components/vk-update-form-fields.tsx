...
export function VkUpdateFormFields({ currentVk, isLoadingCurrentVk }: Props) {
	...
	return (
		<div className="flex flex-col gap-5">
			{/* Current VK */}
			<div>
				<p className="mb-3 text-body font-medium text-[#6b7280]">Current verification key</p>
				{isLoadingCurrentVk ? (
					...
					<div className="rounded-xl border border-[#d97706] bg-[#fffbeb] p-3">
						<div className="flex items-center gap-3 rounded-lg border border-[#e5e7eb] bg-white px-4 py-3">
							...
							{currentVk.conditionHex.length > 0 && (
								<span
									className="min-w-0 flex-1 break-all font-mono text-body text-[#374151]"
			...
			{/* Type selector */}
			<div>
				<p className="mb-3 text-body font-medium text-[#111827]">New predicate type</p>
			...
			{needsCondition && (
				<div>
					<label className="text-body font-medium text-[#111827]">{meta.conditionLabel}</label>
					<input
						type="text"
			...
			{!needsCondition && (
				<div className="rounded-lg border border-[#e5e7eb] bg-[#f9fafb] px-4 py-3 text-body text-[#6b7280]">

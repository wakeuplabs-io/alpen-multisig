...
function StatusLine({ label, reachable, url }: { label: string; reachable: boolean | null; url: string }) {
	if (reachable === null) return null
	return (
		<span className="flex items-center gap-1.5 text-mono-sm text-[#6b7280]">
		...
		>
			<div className="flex max-h-[90vh] w-full max-w-md flex-col rounded-2xl border border-[#e5e7eb] bg-white shadow-xl">
				<div className="flex items-center justify-between border-b border-[#f3f4f6] px-6 py-4">
					<h2 className="text-body-lg font-semibold text-[#0a0a0a]">Node connection</h2>
					<button
						type="button"
						aria-label="Close"
						onClick={onClose}
				...
				<form onSubmit={handleSubmit} className="flex min-h-0 flex-1 flex-col">
					<div className="min-h-0 flex-1 space-y-2 overflow-y-auto px-6 py-5">
						{localNodeStatus && (
							<div className="mb-4 rounded-xl border border-[#e5e7eb] p-4">
								<div className="flex items-center justify-between">
									<p className="text-mono-sm font-medium uppercase tracking-wide text-[#9ca3af]">Connection status</p>
								...
								<div className="mt-2 flex flex-col gap-0.5">
									...
									<button
										...
										className="mt-1 w-fit text-mono-sm text-[#6b7280] underline underline-offset-2 hover:text-[#374151]"
									>
										Recheck
									</button>
								</div>
						...
						<p className="mb-3 text-mono-sm font-medium uppercase tracking-wide text-[#9ca3af]">Node source</p>
							...
							return (
								...
								>
									...
									<div className="flex-1 min-w-0">
										<span className="text-body-sm font-medium text-[#0a0a0a]">{opt.label}</span>
										<p className="mt-0.5 text-label text-[#6b7280]">{opt.description}</p>
									</div>
								</label>
							)
						})}
						...
						{draft.mode === 'local' && (
							<div className="mt-1 space-y-3 rounded-xl border border-[#e5e7eb] p-4">
								<div>
									<label className="mb-1 block text-label font-medium text-[#374151]">Orchestrator URL</label>
									<input
										type="url"
										...
										className="w-full rounded-lg border border-[#e5e7eb] px-3 py-2 text-body-sm text-[#0a0a0a] outline-none focus:border-[#0a0a0a] focus:ring-1 focus:ring-[#0a0a0a]"
									/>
									<p className="mt-1 text-mono-sm text-[#9ca3af]">
						...
						{draft.mode === 'custom' && (
							<div className="mt-1 space-y-3 rounded-xl border border-[#e5e7eb] p-4">
								<div>
									<label className="mb-1 block text-label font-medium text-[#374151]">Strata RPC URL</label>
									<input
										type="url"
								...
								<div>
									<label className="mb-1 block text-label font-medium text-[#374151]">Bitcoin RPC URL</label>
									<input
										type="url"
								...
								<div>
									<label className="mb-1 block text-label font-medium text-[#374151]">Electrum URL</label>
									<input
										type="text"
								...
								<div className="grid grid-cols-2 gap-3">
									<div>
										<label className="mb-1 block text-label font-medium text-[#374151]">RPC User</label>
										<input
											type="text"
											placeholder="rpcuser"
									...
									<div>
										<label className="mb-1 block text-label font-medium text-[#374151]">RPC Password</label>
										<input
											type="password"
											placeholder="rpcpass"
					...
					<div className="border-t border-[#f3f4f6] px-6 py-4">
						<button
							type="submit"
							disabled={isSaving}
							className="w-full rounded-xl bg-[#0a0a0a] py-2.5 text-body-sm font-medium text-white transition hover:bg-[#1f1f1f] disabled:opacity-60"
						>

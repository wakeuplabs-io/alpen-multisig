...
const trezorAdapter = read('wallet/trezor-adapter.ts')
const ledgerAdapter = read('wallet/ledger-adapter.ts')
...
assert.ok(!trezorAdapter.includes('listAddresses'), 'Trezor adapter must not expose derivation picking')
assert.ok(!ledgerAdapter.includes('listAddresses'), 'Ledger adapter must not expose derivation picking')

...
export function useDecodedProposal(proposal: Proposal | null): DecodedProposalData {
  const [sighashHex, setSighashHex] = useState<string | null>(null)
  ...
  useEffect(() => {
    ...
    void Promise.all([
      computeSighash(proposal.seqNo, proposal.actionHex),
      decodeActionHex(proposal.actionHex),
      getMultisigConfig(proposal.authority),
    ]).then(([sighashRes, actionRes, configRes]) => {
      if (cancelled) return
      setIsLoading(false)
      ...
      if (actionRes.ok && actionRes.data.kind === 'multisig_update' && configRes.ok) {
        ...
        if (isEnacted) {
          ...
          beforeSigners = [
            ...config.signers.filter((k) => !addSet.has(k.toLowerCase())),
            ...decoded.removeKeys,
          ]
        } else {
          beforeSigners = config.signers
          afterSigners = [
            ...config.signers.filter((k) => !removeSet.has(k.toLowerCase())),
            ...decoded.addKeys,
          ]
        }
  ...
  return { sighashHex, signerSetChange, allSigners, isLoading }
}

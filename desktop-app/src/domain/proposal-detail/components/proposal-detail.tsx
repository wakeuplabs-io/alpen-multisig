...
export function ProposalDetail({ proposal, signerPubkey, decodedData, onSign, onBroadcast }: Props) {
  ...
  const isTerminal =
    proposal.status === 'enacted' || proposal.status === 'canceled' || proposal.status === 'expired'
  const hasQuorum =
    !isTerminal && (proposal.status === 'approved' || collectedSignatures >= requiredSignatures)
  const alreadySigned =
    signerPubkey !== null &&
        ...
        <div className="p-6 pb-5">
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0 flex-1">
              <h2 className="m-0 font-['BIZ_UDPMincho'] text-[28px] leading-[1.2] text-[#0a0a0a]">
                {title}
              </h2>
                  ...
                  <tr key={i} className="border-b border-[#f3f4f6] last:border-0">
                    ...
                    <td className="px-6 py-3">
                      {row.inAfter ? (
                        ...
                        >
                          {row.isAdded && (
                            <span className="mr-1 text-[#0f9d7a]">+</span>
                          )}
                          {row.pubkey}
                        </span>
                      ) : (
      ...
      <div className="overflow-hidden rounded-xl border border-[#e5e7eb] bg-white shadow-sm">
        <div className="flex items-center justify-between border-b border-[#f3f4f6] px-6 py-4">
          ...
          {proposal.signatures.length > 0 && (
            <CopyButton text={signaturesJson} label="Copy signatures" />
          )}
        </div>
        <div className="divide-y divide-[#f3f4f6]">
          ...
          {proposal.signatures.map((sig, i) => {
            ...
            return (
              <div
                key={i}
                className="flex items-center gap-3 bg-[#f0fdf9] px-6 py-3"
              >
                ...
                <span className="flex-1 font-mono text-[12px] text-[#111827]">
                  {truncatePubkey(sig.signerPubkey)}
                </span>
                {isMe && (
          ...
          {decodedData.allSigners
            .filter(
              (signer) =>
                !proposal.signatures.some((s) => s.signerPubkey.toLowerCase() === signer.toLowerCase()),
            )
            .map((signer, i) => (
              <div key={`pending-${i}`} className="flex items-center gap-3 px-6 py-3">
                ...
                <span className="flex-1 font-mono text-[12px] text-[#6b7280]">

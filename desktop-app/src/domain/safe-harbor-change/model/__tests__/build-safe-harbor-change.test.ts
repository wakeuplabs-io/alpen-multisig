...
import { buildSafeHarborChange } from '../build-safe-harbor-change.ts'
...
const pending = buildSafeHarborChange({ installed: INSTALLED, proposed: PROPOSED, isEnacted: false })
...
const enacted = buildSafeHarborChange({ installed: PROPOSED, proposed: PROPOSED, isEnacted: true })
assert.deepEqual(
	enacted,
...
assert.equal(
	buildSafeHarborChange({ installed: null, proposed: PROPOSED, isEnacted: false }),
	null,
	'claim 3: an unreadable installed destination suppresses the section rather than degrading it',
)

console.log('build-safe-harbor-change: all assertions passed')

...
export function Paginator({ page, totalPages, onPageChange, className }: Props) {
	...
	return (
		<div className={className ?? 'mt-1 flex items-center justify-center gap-1.5'}>
			...
			<span className="min-w-8 rounded border border-[#d1d5db] bg-white px-2 py-0.5 text-center text-[12px] font-medium text-[#374151]">
				{page}
			</span>
...
function PaginatorButton({
	...
	return (
		<button
			...
			disabled={disabled}
			onClick={onClick}
			className="flex h-6 w-6 items-center justify-center rounded border border-[#e5e7eb] bg-white text-[12px] text-[#6b7280] transition hover:border-[#d1d5db] hover:bg-[#f9fafb] hover:text-[#374151] disabled:cursor-not-allowed disabled:opacity-40"
		>
			{children}
		</button>
	)

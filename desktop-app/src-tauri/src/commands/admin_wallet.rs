...
pub async fn wallet_session_init_watch_only(
    ...
    if let Some(fp) = input.master_fingerprint {
        let device_type = match input.device_type.as_deref() {
            Some("trezor") => desktop_app::infrastructure::hw_wallet::hw_psbt_signer::HwDeviceType::Trezor,
            Some("ledger") => desktop_app::infrastructure::hw_wallet::hw_psbt_signer::HwDeviceType::Ledger,
            _ => return Err("device_type must be 'trezor' or 'ledger' when master_fingerprint is provided".to_string()),
        };
        wallet_session

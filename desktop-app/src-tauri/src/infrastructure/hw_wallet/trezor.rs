...
// ─── BIP-84 first receive address (matches POC JS adapter) ───────────────────
const DEFAULT_PATH: &str = "m/84'/0'/0'/0/0";
...
pub fn connect(derivation_path: Option<String>) -> Result<HwWalletInfo, String> {
	...
	let xpub = resolve(
		trezor
			.get_public_key(&path, InputScriptType::SPENDADDRESS, bitcoin::Network::Bitcoin, false)
	...
	let address: bitcoin::Address = resolve(
		trezor
			.get_address(&path, InputScriptType::SPENDADDRESS, bitcoin::Network::Bitcoin, false)
...
pub fn sign_message(sighash_hex: &str, derivation_path: &str) -> Result<SignatureResult, String> {
	...
	Ok(SignatureResult { public_key_hex, signature_hex })
}

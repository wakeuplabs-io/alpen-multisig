...
fn serialize_recoverable_signature(signature: RecoverableSignature) -> [u8; 65] {
...
/// Signs the provided 32-byte digest as a Bitcoin message on Trezor.
pub fn sign_admin_sps65_binding(
    sighash_hex: &str,
    derivation_path: &str,
) -> Result<SignatureResult, String> {
    let path = parse_path(derivation_path)?;
    decode_sighash_32(sighash_hex)?;
    let mut trezor = open_trezor()?;
    ...
    let signature_bytes = serialize_recoverable_signature(signature);

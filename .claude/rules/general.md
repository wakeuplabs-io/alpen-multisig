...
- **Rust**: `cargo fmt` for formatting, `cargo clippy -- -D warnings` for linting (zero tolerance for warnings)
- **Frontend**: Prettier for formatting (`desktop-app/.prettierrc`), ESLint for linting (`desktop-app/eslint.config.js`)
